...
the final test accuracy will be about 91.93% for VGG-Net. We use this script for spike counting.
...
with tf.device('/cpu:0'):
    ...
    def model(x_crop, y_, is_train, reuse):
        ...
        with tf.variable_scope("model", reuse=reuse):
            net, spikes = tl.models.VGG_CIFAR10_spike(x_crop, y_, pretrained=False, k=k, B=B, end_with='fc1000', n_classes=10, is_train=is_train, reuse=reuse, name=None)
            ...
            net = tl.layers.BatchNormLayer(net, act=tf.identity, is_train=is_train, name='bn_out')
    ...
    with tf.device('/gpu:0'):  # <-- remove it if you don't have GPU
        network, cost, acc, _ = model(x_train_batch, y_train_batch, True, False)
        _, cost_test, acc_test, spikes = model(x_test_batch, y_test_batch, False, True)
