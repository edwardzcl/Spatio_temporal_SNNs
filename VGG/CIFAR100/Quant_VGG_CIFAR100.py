...
# -*- coding: utf-8 -*-
"""
- 1. This model has 5079272 paramters and "Median Quantization" strategy(weight:ternary {-1,0,+1}, active: quantization level = 1, upper bound = 2),
you can manually decay the learning rate from original 0.01 by 10× every 200 epochs while restore from corresponding checkpoint by setting flag "resume = True", 
the final test accuracy will be about 91.93% for VGG-Net. We use this script for quantization and training.
...
.. https://github.com/tensorflow/tensorflow/tree/r0.9/tensorflow/models/image/cifar100
Note
------
...
# resume from previous checkpoint after 200 epochs and decay learning rate by 10 times
...
print(args.k, args.B, args.resume, args.learning_rate, args.mode)
...
model_file_name = "./model_cifar100_advanced.ckpt"
...
with tf.device('/cpu:0'):
    ...
    def model(x_crop, y_, is_train, reuse):
        ...
        with tf.variable_scope("model", reuse=reuse):
            net = tl.models.VGG_CIFAR100(x_crop, y_, pretrained=False, k=args.k, B=args.B, end_with='fc1000', n_classes=100, is_train=is_train, reuse=reuse, name=None)
            net = tl.layers.FlattenLayer(net, name='flatten')

            # the last layer is usually high-precison refer to the previous works.
            net = tl.layers.DenseLayer(net, 100, b_init=None, name='output')
    ...
    if args.mode == 'training':
       ...
       for epoch in range(n_epoch):
           ...
           if (epoch + 1) % (print_freq * 50) == 0:
               ...
               tl.files.save_npz(network.all_params, name='model_cifar_100.npz', sess=sess)
