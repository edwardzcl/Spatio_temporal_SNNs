...
def model(x, is_train=True, reuse=False):
    ...
    with tf.variable_scope("binarynet", reuse=reuse):
        ...
        net = tl.layers.Conv2d(net, n_filter=16, filter_size=(5, 5), strides=(1, 1), padding='VALID', b_init=None, name='bcnn1')
        net = tl.layers.BatchNormLayer(net, act=tf.nn.relu, is_train=is_train, name='bn1')

        net = tl.layers.Conv2d(net, n_filter=16, filter_size=(2, 2), strides=(2, 2), padding='SAME', b_init=None, name='bcnn2')
        #net = tl.layers.MaxPool2d(net, (2, 2), (2, 2), padding='SAME', name='pool1')
        net = tl.layers.BatchNormLayer(net, act=tf.nn.relu, is_train=is_train, name='bn2')

        net = tl.layers.Conv2d(net, n_filter=32, filter_size=(5, 5), strides=(1, 1), padding='VALID', b_init=None, name='bcnn3')
        #net = tl.layers.MaxPool2d(net, (2, 2), (2, 2), padding='SAME', name='pool2')
        net = tl.layers.BatchNormLayer(net, act=tf.nn.relu, is_train=is_train, name='bn3')

        net = tl.layers.Conv2d(net, n_filter=32, filter_size=(2, 2), strides=(2, 2), padding='SAME', b_init=None, name='bcnn4')
        #net = tl.layers.MaxPool2d(net, (2, 2), (2, 2), padding='SAME', name='pool2')
